 ...
 * @param {boolean} v2 - Use v2 editor.
 * @param {(projectUrl: string) => void} callback - The callback
 * @returns {Promise<{ errors: string[], sceneId: number }>} The data result.
 */
export const visitEditor = async (page, outPath, projectId, v2 = false, callback = () => {}) => {
    const projectUrl = editorProjectUrl(projectId, v2);
    let sceneId = 0;
    const errors = await capture({
        page,
        outPath,
 ...
 */
export const visitEditorScene = async (page, outPath, sceneId, v2 = false, callback = () => {}) => {
    const sceneUrl = editorSceneUrl(sceneId, v2);
    const errors = await capture({
        page,
        outPath,
        callback: async () => {
