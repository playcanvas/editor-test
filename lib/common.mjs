 ...
 * @returns {Promise<{ errors: string[], projectId: string }>} The errors.
 */
...
export const importProject = async (page, outPath, importPath) => {
    ...
    const errors = await capture({
        ...
        callback: async () => {
            ...
            const importProjectPromise = page.evaluate(() => wi.importProject());
 ...
 * @returns {Promise<{ errors: string[], sceneId: string }>} The errors.
 */
export const visitEditor = async (page, outPath, projectId, callback = () => {}) => {
    ...
    const errors = await capture({
        ...
        callback: async () => {
            ...
            sceneId = /scene\/(\d+)/.exec(page.url())[1];
